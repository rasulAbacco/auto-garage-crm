...
import { FiEye, FiEdit, FiTrash2, FiX, FiUser, FiPhone, FiMail, FiCalendar } from "react-icons/fi";
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'} lg:ml-16`}>
            <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
                {/* Error Message */}
                {error && (
                    <div className={`rounded-2xl p-4 shadow-lg border ${isDark ? "bg-red-900/20 border-red-800" : "bg-red-50 border-red-200"}`}>
                ...
                {/* Header */}
                <div className={`rounded-2xl p-6 sm:p-8 shadow-lg ${isDark ? 'bg-gradient-to-r from-blue-900/50 to-purple-900/50' : 'bg-gradient-to-r from-blue-600 to-purple-600'}`}>
                ...
                {loading ? (
                    <div className={`rounded-2xl p-16 text-center shadow-lg ${isDark ? "bg-gray-800" : "bg-white"}`}>
                ...
                {/* Pagination */}
                <div className={`rounded-2xl p-4 sm:p-6 shadow-lg ${isDark ? "bg-gray-800" : "bg-white"}`}>
...
function ClientCard({ client, onView, onEdit, onDelete, onQuickView, isDark }) {
    // Get last service date from services array if available
    const lastServiceDate = client.services && client.services.length > 0
        ? new Date(client.services[0].date).toLocaleDateString()
        : null;
            ...
            <div className="flex flex-col md:flex-row">
                ...
                <div className="flex-1 p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                    ...
                    <div className="flex-1 space-y-4">
                        {/* Client Info */}
                        <div className="flex items-center gap-3">
                            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xl font-bold shadow-lg flex-shrink-0">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            <div className={`flex items-center gap-2 text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                                <FaCar size={14} />
                                <span>Year: {client.vehicleYear || 'N/A'}</span>
                            </div>
                            ...
                            <div className={`flex items-center gap-2 text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                                <FiCalendar size={14} className={isDark ? "text-green-400" : "text-green-600"} />
                                <span>Last Service: {lastServiceDate || 'N/A'}</span>
                            </div>
                            <div className={`flex items-center gap-2 text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                                <span className={`font-mono font-bold px-3 py-1 rounded-lg ${isDark ? "bg-gray-700 text-yellow-400" : "bg-gray-100 text-gray-900"}`}>
                                    {client.regNumber}
                                </span>
                            </div>
                        </div>
                        ...
                        {client.vin && (
                            <div className={`flex items-center gap-2 text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                                ...
                                <span className="font-mono">VIN: {client.vin}</span>
                            </div>
                        )}
                    </div>
...
function ClientModal({ client, onClose, navigate, deleteClient, isDark }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
            <div className={`rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col ${isDark ? "bg-gray-800" : "bg-white"}`}>
                {/* Header */}
                ...
                <div className="overflow-y-auto flex-1">
                    <div className="p-6 space-y-6">
                        {/* Vehicle Image */}
                        <div className={`rounded-2xl overflow-hidden shadow-lg ${isDark ? "bg-gray-700" : "bg-gray-100"}`}>
                            <img
                        ...
                        {/* Client Info */}
                        <div className={`flex items-center gap-4 p-5 rounded-2xl ${isDark ? "bg-gray-700" : "bg-gray-50"}`}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <InfoItem icon={<FiMail />} label="Email" value={client.email || 'N/A'} isDark={isDark} />
                            <InfoItem icon={<FaCar />} label="Vehicle" value={`${client.vehicleMake} ${client.vehicleModel}`} isDark={isDark} />
                            <InfoItem icon={<FaCar />} label="Year" value={client.vehicleYear || 'N/A'} isDark={isDark} />
                            <InfoItem
                                icon={
                            ...
                            <InfoItem
                                ...
                                label="VIN / Chassis"
                                value={client.vin || 'Not Provided'}
                                isDark={isDark}
                            />
                            <InfoItem
                                icon={<FiCalendar />}
                                label="Last Service"
                                value={lastServiceDate || 'No Service Yet'}
                                isDark={isDark}
                            />
                        </div>

                        {/* Receiver Name if exists */}
                        {client.receiverName && (
                            <div className={`p-4 rounded-xl ${isDark ? "bg-gray-700" : "bg-gray-50"}`}>
                                <div className={`text-xs font-medium mb-1 ${isDark ? "text-gray-400" : "text-gray-500"}`}>Receiver Name</div>
                                <div className={`font-semibold ${isDark ? "text-white" : "text-gray-900"}`}>{client.receiverName}</div>
                            </div>
                        )}
                    </div>
                </div>
...
function InfoItem({ icon, label, value, isDark }) {
    return (
        <div className={`flex items-start gap-3 p-4 rounded-xl ${isDark ? "bg-gray-700" : "bg-white"}`}>
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white flex-shrink-0 shadow">
                {icon}
            </div>
            <div className="min-w-0 flex-1">
