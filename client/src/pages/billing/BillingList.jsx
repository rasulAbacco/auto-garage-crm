...
// Payment mode configuration with colors and icons
const paymentModes = {
  'cash': { emoji: '💵', label: 'Cash', color: 'green' },
  'credit-card': { emoji: '💳', label: 'Credit Card', color: 'blue' },
  'debit-card': { emoji: '💳', label: 'Debit Card', color: 'purple' },
  'bank-transfer': { emoji: '🏦', label: 'Bank Transfer', color: 'indigo' },
  'check': { emoji: '📝', label: 'Check', color: 'yellow' },
  'online': { emoji: '🌐', label: 'Online Payment', color: 'teal' },
  'default': { emoji: '💰', label: 'Not Specified', color: 'gray' }
};
...
const InvoiceCard = ({ invoice, customer, onView, onDelete, index }) => {
  const { isDark } = useTheme();
  const [showActions, setShowActions] = useState(false);

  const paymentMode = paymentModes[invoice.mode?.toLowerCase()] || paymentModes.default;
          ...
          <div className="flex-shrink-0 relative">
            <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600 flex items-center justify-center shadow-xl transform transition-transform group-hover:scale-110 group-hover:rotate-3">
              <div className="text-center">
                ...
                <p className="text-white text-xs font-bold tracking-wider">#{String(invoice.id).padStart(4, '0')}</p>
              </div>
            </div>
            {/* Floating badge */}
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border-2 ${isDark
                      ? 'bg-gray-700/50 text-gray-300 border-gray-600'
                      : 'bg-gray-50 text-gray-700 border-gray-200'
                    }`}>
                    <span className="text-lg">{paymentMode.emoji}</span>
                    {paymentMode.label}
                  </span>
                </div>
              </div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm">
                  {customer.phone && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className={isDark ? 'text-gray-300' : 'text-gray-700'}>{customer.phone}</span>
                    </div>
                  )}
                  {customer.email && (
                    <div className="flex items-center gap-2">
                      <FiMail className={isDark ? 'text-purple-400' : 'text-purple-600'} size={14} />
                      <span className={isDark ? 'text-gray-300' : 'text-gray-700'}>{customer.email}</span>
                    </div>
                  )}
                </div>
              </div>
...
export default function BillingList() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(b => {
      const matchesSearch = [
        ...
        b.description,
        b.date,
        clients.find(c => c.id === Number(b.customerId))?.fullName || ''
  ...
  }, [q, data, clients, filterStatus])

  // Calculate statistics
  const stats = useMemo(() => {
    const totalRevenue = data.reduce((sum, inv) => sum + Number(inv.total || 0), 0);
    const paidInvoices = data.filter(inv => inv.status?.toLowerCase() === 'paid');
    const paidRevenue = paidInvoices.reduce((sum, inv) => sum + Number(inv.total || 0), 0);
    const pendingInvoices = data.filter(inv => inv.status?.toLowerCase() !== 'paid');
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`font-bold ${isDark ? 'text-green-400' : 'text-green-600'}`}>
                  ${filtered.reduce((sum, inv) => sum + Number(inv.total || 0), 0).toFixed(2)}
                </span>
              </div>
            </div>
          </div>
